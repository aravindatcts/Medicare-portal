...
export const MobileClaimsHero = () => <ClaimsAiConcierge />;

export const MobileClaimsFilter = ({ context }: { context?: any }) => {
...
const getIconStyles = (type: string) => {
  ...
  if (t.includes('dental')) return { icon: 'tooth-outline', bg: '#eff6ff', color: '#2563eb' };
  if (t.includes('vision')) return { icon: 'eye-outline', bg: '#fdf4ff', color: '#c026d3' };
  if (t.includes('pharmacy') || t.includes('meds')) return { icon: 'pill', bg: '#ccfbf1', color: '#0f766e' };
  if (t.includes('medical') || t.includes('outpatient') || t.includes('visit') || t.includes('consult') || t.includes('services') || t.includes('therapy') || t.includes('imaging') || t.includes('study')) {
...
export const MobileClaimsList = ({ context }: { context?: any }) => {
  const { data: claims, isLoading } = useClaims();
  const PAGE_SIZE = 5;
  const { activeFilters, currentPage, setCurrentPage, onNavigate } = context || {};
  ...
  const paginatedClaims = displayedClaims.slice((safePage - 1) * PAGE_SIZE, safePage * PAGE_SIZE);
    ...
    <>
      <View style={styles.claimsList}>
        {paginatedClaims.map((claim: any) => (
          <ClaimCard
            key={claim.id}
            claim={{...claim, onPress: (id: string) => onNavigate?.('ClaimDetail', { claimId: id })}}
          />
        ))}
      </View>
      ...
      {totalPages > 1 && (
        <View style={styles.pagination}>
          <TouchableOpacity 
            style={[styles.pageBtn, safePage === 1 && { opacity: 0.5 }]}
          ...
          <TouchableOpacity 
            style={[styles.pageBtn, safePage === totalPages && { opacity: 0.5 }]}
