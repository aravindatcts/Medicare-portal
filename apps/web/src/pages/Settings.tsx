...
import { useSettings, useCmsPage } from '@medicare/shared';
...
export default function Settings() {
  ...
  const { data: cmsPage, isLoading: cmsLoading } = useCmsPage('settings');
  const isLoading = settingsLoading || cmsLoading;

  if (isLoading) {
    return (
      <main className={styles.pageWrap}>
        <section className="mb-16">
          <div className={styles.skeleton} style={{ height: 52, width: 360, borderRadius: 10, marginBottom: 16 }} />
          <div className={styles.skeleton} style={{ height: 20, width: 520, borderRadius: 6 }} />
        </section>
        <div className="space-y-12">
          <div className={styles.skeleton} style={{ height: 280, borderRadius: 24 }} />
          <div className={styles.skeleton} style={{ height: 400, borderRadius: 24 }} />
        </div>
      </main>
    );
  }
