...
export const WebClaimsFilter = ({ context }: { context?: any }) => {
  const { 
...
export const WebClaimsList = ({ context }: { context?: any }) => {
  const navigate = useNavigate();
  ...
  const filteredClaims = useMemo(() => {
    ...
    if (filterDateRange && filterDateRange !== 'all') result = result.filter(c => claimMatchesDateRange(c, filterDateRange));
    return result;
    ...
    <>
      ...
      {!isLoading && totalClaims === 0 && (
        <div className="text-center py-20 text-on-surface-variant">
          ...
          <button onClick={() => {
            if (context?.setFilterStatus) context.setFilterStatus('');
            if (context?.setFilterDateRange) context.setFilterDateRange('all');
            if (context?.setFilterProvider) context.setFilterProvider('');
            if (context?.setCurrentPage) context.setCurrentPage(1);
